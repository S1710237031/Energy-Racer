...
/// <summary>
/// manages all dots
/// </summary> public class Dot : MonoBehaviour {     private Vector2 firstTouch;     private Vector2 secondTouch;     public float swipe = 0; //angle     public int row;     public int col;     private GameObject swapWith;     private Board board;     public int targetX;     public int targetY;     private Vector2 tempPos;     public bool isMatched = false;     public float swipeResist = 1f;     private FindMatches findMatches;

    /// <summary>
    /// set position of dot
    /// </summary>     void Start()     {         board = FindObjectOfType<Board>();         targetX = (int)transform.position.x;         targetY = (int)transform.position.y;         row = targetY;         col = targetX;         findMatches = FindObjectOfType<FindMatches>();     }
    ...
    /// </summary>     void Update()     {
        ...
        if (isMatched)
        {
            ...
            }             findMatches.findAllMatches();         }         else         {             tempPos = new Vector2(targetX, transform.position.y);             transform.position = tempPos;             board.allDots[col, row] = this.gameObject;         }          if (Mathf.Abs(targetY - transform.position.y) > .1)
        {             tempPos = new Vector2(transform.position.x, targetY);
            transform.position = Vector2.Lerp(transform.position, tempPos, .6f);             if (board.allDots[col, row] != this.gameObject)             {                 board.allDots[col, row] = this.gameObject;             }
            findMatches.findAllMatches();          }
        else         {             tempPos = new Vector2(transform.position.x, targetY);             transform.position = tempPos;         }     }
    ...
    /// </summary>
    /// <returns></returns>     public IEnumerator CheckMoveCo()
    {
